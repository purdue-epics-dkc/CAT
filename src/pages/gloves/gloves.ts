...
export class GlovesPage {
  ...
  scan() {
    this.ble.scan([DeafSigner.device], 10).subscribe(
      (service) => {
        this.onScan(service);
      },
  ...
  onScan(service: Observable<any>) {
    ...
    this.ble.connect(service.id).subscribe(
      (bleStatus) => {
        this.onConnect();
      },
      (err) => {
  ...
  onConnect() {
    ...
    this.ble.writeWithoutResponse(DeafSigner.device, DeafSigner.service, DeafSigner.dataEnable, [1]);

    // Read all characteristics when RightHand sends a notification.
    this.ble.startNotification(DeafSigner.device, DeafSigner.service, DeafSigner.rightHand).subscribe(
      (data) => {
